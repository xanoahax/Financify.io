...
const FREQUENCIES: Array<{ value: HouseholdCostFrequency; de: string; en: string }> = [
  { value: 'weekly', de: 'WÃ¶chentlich', en: 'Weekly' },
  { value: 'biweekly', de: '2-wÃ¶chentlich', en: 'Biweekly' },
  { value: 'monthly', de: 'Monatlich', en: 'Monthly' },
  { value: 'yearly', de: 'JÃ¤hrlich', en: 'Yearly' },
  { value: 'one_time', de: 'Einmalig', en: 'One-time' },
]

const SPLIT_TYPES: Array<{ value: HouseholdSplitType; de: string; en: string }> = [
  { value: 'equal', de: 'GleichmÃ¤Ãig', en: 'Equal' },
...
const CATEGORY_LABELS: Record<string, { de: string; en: string }> = {
  ...
  Mobility: { de: 'MobilitÃ¤t', en: 'Mobility' },
  Maintenance: { de: 'RÃ¼cklagen/Instandhaltung', en: 'Maintenance' },
...
const SUBCATEGORY_LABELS: Record<string, { de: string; en: string }> = {
  ...
  'Building insurance': { de: 'GebÃ¤udeversicherung', en: 'Building insurance' },
  ...
  'Waste disposal': { de: 'MÃ¼llentsorgung', en: 'Waste disposal' },
  Supermarket: { de: 'Supermarkt', en: 'Supermarket' },
  'Household goods': { de: 'Haushaltswaren', en: 'Household goods' },
  'Public transport': { de: 'Ãffi-Ticket', en: 'Public transport' },
...
export function HouseholdsPage(): JSX.Element {
  ...
  const activeHouseholdId = useMemo(() => {
    ...
    if (selectedHouseholdId && sortedHouseholds.some((household) => household.id === selectedHouseholdId)) {
      return selectedHouseholdId
    }
    return sortedHouseholds[0].id
  }, [selectedHouseholdId, sortedHouseholds])

  const selectedHousehold = useMemo(() => sortedHouseholds.find((h) => h.id === activeHouseholdId) ?? null, [activeHouseholdId, sortedHouseholds])
  const members = useMemo(() => householdMembers.filter((m) => m.householdId === activeHouseholdId), [activeHouseholdId, householdMembers])
  const payers = useMemo(() => householdPayers.filter((p) => p.householdId === activeHouseholdId), [activeHouseholdId, householdPayers])
  const costs = useMemo(() => householdCosts.filter((c) => c.householdId === activeHouseholdId), [activeHouseholdId, householdCosts])
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      if (quickAdd === 'household') {
        setHouseholdForm({ name: '', type: 'rental', currency: settings.currency, billingStart: todayString() })
        setHouseholdOpen(true)
      } else if (quickAdd === 'member') {
        if (activeHouseholdId) {
          setMemberForm({ householdId: activeHouseholdId, name: '', role: '', activeFrom: todayString() })
          setMemberOpen(true)
        } else {
          setHouseholdForm({ name: '', type: 'rental', currency: settings.currency, billingStart: todayString() })
          setHouseholdOpen(true)
        }
      } else if (quickAdd === 'payer') {
        if (activeHouseholdId) {
          setPayerForm({ householdId: activeHouseholdId, name: '' })
          setPayerOpen(true)
        } else {
          setHouseholdForm({ name: '', type: 'rental', currency: settings.currency, billingStart: todayString() })
          setHouseholdOpen(true)
        }
      } else if (quickAdd === 'cost') {
        if (activeHouseholdId) {
          setEditCostId(null)
          setEffectiveFromDate(todayString())
          setCostForm(buildDefaultCostForm(activeHouseholdId))
          setCostOpen(true)
        } else {
          setHouseholdForm({ name: '', type: 'rental', currency: settings.currency, billingStart: todayString() })
          setHouseholdOpen(true)
  ...
  }, [activeHouseholdId, searchParams, setSearchParams, settings.currency])
  ...
  async function submitMember(event: React.FormEvent): Promise<void> { event.preventDefault(); try { await addHouseholdMember({ householdId: memberForm.householdId, name: memberForm.name, role: memberForm.role, activeFrom: memberForm.activeFrom, activeTo: null, isActive: true }); setMemberOpen(false); setMemberForm({ householdId: activeHouseholdId, name: '', role: '', activeFrom: todayString() }); setErrorText('') } catch (error) { setErrorText(error instanceof Error ? error.message : t('Mitglied konnte nicht gespeichert werden.', 'Member could not be saved.')) } }
  async function submitPayer(event: React.FormEvent): Promise<void> { event.preventDefault(); try { await addHouseholdPayer({ householdId: payerForm.householdId, name: payerForm.name, type: 'external', linkedMemberId: '' }); setPayerOpen(false); setPayerForm({ householdId: activeHouseholdId, name: '' }); setErrorText('') } catch (error) { setErrorText(error instanceof Error ? error.message : t('Zahler konnte nicht gespeichert werden.', 'Payer could not be saved.')) } }
  async function submitCost(event: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      setCostOpen(false); setEditCostId(null); setEffectiveFromDate(todayString()); setCostForm(buildDefaultCostForm(activeHouseholdId)); setErrorText('')
    } catch (error) {
  ...
  return (
    <section ref={pageRef} className="page">
      ...
      <header className="page-header page-header-compact">
          ...
          <div className="shell-plus-cluster" ref={quickActionsRef}>
            ...
            {quickActionsOpen ? (
              <div className="shell-plus-menu" role="menu" aria-label={t('Schnellaktionen', 'Quick actions')}>
                <button type="button" className="shell-plus-menu-item" role="menuitem" onClick={() => { setQuickActionsOpen(false); setHouseholdOpen(true); setHouseholdForm({ name: '', type: 'rental', currency: settings.currency, billingStart: todayString() }) }}>
                  {t('Haushalt hinzufügen', 'Add household')}
                </button>
                <button type="button" className="shell-plus-menu-item" role="menuitem" disabled={!activeHouseholdId} onClick={() => { setQuickActionsOpen(false); setMemberOpen(true); setMemberForm({ householdId: activeHouseholdId, name: '', role: '', activeFrom: todayString() }) }}>
                  {t('Mitglied hinzufügen', 'Add member')}
                </button>
                <button type="button" className="shell-plus-menu-item" role="menuitem" disabled={!activeHouseholdId} onClick={() => { setQuickActionsOpen(false); setPayerOpen(true); setPayerForm({ householdId: activeHouseholdId, name: '' }) }}>
                  {t('Externer Zahler', 'External payer')}
                </button>
                <button type="button" className="shell-plus-menu-item" role="menuitem" disabled={!activeHouseholdId} onClick={() => { setQuickActionsOpen(false); setEditCostId(null); setEffectiveFromDate(todayString()); setCostOpen(true); setCostForm(buildDefaultCostForm(activeHouseholdId)) }}>
                  {t('Kosten hinzufügen', 'Add cost')}
                </button>
              </div>
            ) : null}
          </div>
        ...
        <article className="card stat-card"><p className="muted">{t('Gesamtkosten (monatlich)', 'Total costs (monthly)')}</p><p className="stat-value"><AnimatedNumber value={monthlyGross} formatter={(value) => formatMoney(value, settings.currency, settings.privacyHideAmounts)} /></p></article>
        <article className="card stat-card"><p className="muted">{t('Von Bewohnern zu tragen', 'Resident share')}</p><p className="stat-value"><AnimatedNumber value={monthlyResidents} formatter={(value) => formatMoney(value, settings.currency, settings.privacyHideAmounts)} /></p></article>
        <article className="card stat-card"><p className="muted">{t('Extern bezahlt', 'Paid by externals')}</p><p className="stat-value"><AnimatedNumber value={monthlyExternal} formatter={(value) => formatMoney(value, settings.currency, settings.privacyHideAmounts)} /></p></article>
        <article className="card stat-card"><p className="muted">{t('Bewohner', 'Residents')}</p><p className="stat-value"><AnimatedNumber value={members.length} formatter={(value) => Math.round(value).toString()} /></p></article>
      </div>
      </section>
      ...
      <article className="card">
        <header className="section-header">
          ...
          <div className="filters">
            <select value={activeHouseholdId} onChange={(event) => setSelectedHouseholdId(event.target.value)}>{sortedHouseholds.map((h) => <option key={h.id} value={h.id}>{h.name}</option>)}</select>
            {selectedHousehold ? <button type="button" className="button button-danger" onClick={() => setDeleteTarget({ kind: 'household', id: selectedHousehold.id, label: selectedHousehold.name })}>{t('LÃ¶schen', 'Delete')}</button> : null}
          </div>
        </header>
      </article>
      ...
      <div className="two-column two-column-equal">
        <article className="card"><h2>{t('Kategorienverteilung', 'Category breakdown')}</h2><DonutChart data={categorySeries} language={settings.language} valueFormatter={(value) => formatMoney(value, settings.currency, settings.privacyHideAmounts)} reverseColorScale /></article>
      ...
      <article className="card"><h2>{t('Trend (12 Monate)', 'Trend (12 months)')}</h2><LineChart data={trend} language={settings.language} valueFormatter={(value) => formatMoney(value, settings.currency, settings.privacyHideAmounts)} reverseColorScale /></article>

      <article className="card">
        <h2>{t('KostenÃ¼bersicht', 'Cost overview')}</h2>
        <div className="table-wrap"><table><thead><tr><th>{t('Titel', 'Title')}</th><th>{t('Kategorie', 'Category')}</th><th>{t('Intervall', 'Frequency')}</th><th>{t('Zahler', 'Payer')}</th><th>{t('Monatlich', 'Monthly')}</th><th>{t('Aktionen', 'Actions')}</th></tr></thead><tbody>
          {tableCosts.map((cost) => (
            <tr key={cost.id}>
              ...
              <td>{CATEGORY_LABELS[cost.category]?.[language] ?? cost.category} Â· {SUBCATEGORY_LABELS[cost.subcategory]?.[language] ?? cost.subcategory}</td>
